...
# Palavras-chave de classificação corrigidas
...
def parse_message(text, enviado_por):
    ...
    for line in lines:
        ...
        # Identificar seção principal (usando regex para capturar variações de palavras-chave)
        for keyword in classification_keywords:
            if re.search(r'\b' + re.escape(keyword.lower()) + r'\b', line.lower()):
                current_section = keyword
                break

        # Identificar subseção (quando a linha começa com "-")
        if line.startswith('-'):
        ...
        try:
            ...
            if not filtered_df.empty:
                ...
                if all_data:
                    ...
                    # Mostrar dados em diferentes abas
                    tabs = st.tabs(
                    ...
                    st.write("### Distribuição por Data")
                    data_count = consolidated_df.groupby(consolidated_df['Data'].dt.date).size()
                    st.line_chart(data_count)
